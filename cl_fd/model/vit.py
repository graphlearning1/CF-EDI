...
class VisionTransformer(nn.Module):
    ...
    def random_masking(self, x, epoch):
        """
    ...
    def pos_token(self, x, epoch):
        x = x + self.pos_embed[:, 1:, :]

        # masking: length -> length * mask_ratio
        x, mask, ids_restore = self.random_masking(x, epoch)
    ...
    def forward_features(self, x, epoch, mom):
        x = self.patch_embed(x)
        if mom:
            x = self._pos_embed(x)
        else:
            x, mask, ids_restore = self.pos_token(x, epoch)
        x = self.norm_pre(x)
    ...
    def mask_patches(self, patches):
        ...
        dim = patches.shape[2]
        probability_matrix = torch.full((batchsize, num_patches - 1, dim), 0.125)
        masked_patch = torch.bernoulli(probability_matrix).bool()
        masked_cls = torch.zeros((batchsize, 1, dim), dtype=bool)
        masked_indices = torch.cat((masked_cls, masked_patch), dim=1)
        patches[masked_indices] = 0
        return patches 
    ...
    def forward(self, x, epoch, mom):
        if mom:
            x = self.forward_features(x, epoch, mom)
        else:
            x, mask, ids_restore = self.forward_features(x, epoch, mom)
        patches = x
        x = self.forward_head(x)
        if mom:
            return x, patches
        else:
            return x, patches, mask, ids_restore
